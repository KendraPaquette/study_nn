...
### Problem Formulation
We consider the framework in the context of partially observed Markov Decision Processes. 
...
New trajectories are with the current policy π, which takes an engineered feature φ(ht) as an input and outputs the next action. π is updated to maximize the long-term returns of the system. <!--update math notation-->
...
Prediction network takes an engineered representation of histories and predict the lapse event within the 24 hours window. 
...
1. $(P(Y=1| \text{partial surveys}) - P(Y=1| \text{all survey}))^2$

2. $Y * \log P(Y=1| \text{partial surveys}) + (1-Y) * \log P(Y=0| \text{partial surveys})$ - this is from Yoon et al.
...
Another baseline: Random dropout (66%, 90%)
