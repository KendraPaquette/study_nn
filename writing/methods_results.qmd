...
freq |> 
  ...
  labs(title = "Histogram of proportion of surveys used by subid for each penalty",
       x = "proportion of surveys completed")
```

Plot below is not right because id in model output does not much subid so cant match to days on study.
```{r}
freq |>
  filter(model != "full") |> 
  left_join(lapses, by = "subid") |> 
  ggplot(aes(x = prop, y = prop_lapse_days), alpha = .4) +
  geom_point() +
  facet_wrap(~model) +
